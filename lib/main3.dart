...
class _MyHomePageState extends State<MyHomePage> {
  ...
  late FaceRecognitionWorker _faceRecognitionWorker;
  ...
  void createWorker() async {
    _faceRecognitionWorker = await FaceRecognitionWorker.create();
  }
      ...
      await controller.initialize().then((_) {
        ...
        controller.startImageStream((image) async {
          if (!isDetecting) {
            ...
            recognitions = await _faceRecognitionWorker.recognitionWithImage(image);
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                  onPressed: () {
                    MyFaceFaceRecognition.registerFaceInDB(textEditingController.text, recognition.embeddings);
...
class FaceRecognitionWorker {
  ...
  Future<List<Recognition>> recognitionWithImage(CameraImage cameraImage) async {
  ...
  static Future<FaceRecognitionWorker> create() async {
  ...
  static _communicator(SendPort sp) async {
    ...
    rp.listen((message) async {
      ...
      if(message is RootIsolateToken){
        ...
        final result = await _recognitionWithImage(message);
        sp.send(result);
      }
    });
  }
  ...
  static Future<List<Recognition>> _recognitionWithImage(CameraImage cameraImage) async {
    ...
    // 识别人脸
    List<Recognition> recognitions = await MyFaceFaceRecognition.getFaceRecognitionFromFaces(cropImage, cameraImage);
...
class MyFaceFaceRecognition {
  ...
  static getFaceRecognitionFromFaces(List<Face> faces, CameraImage cameraImage) {

    final time1 = DateTime.now().millisecondsSinceEpoch;

    List<Recognition> recognitions = [];
    // 耗时严重
    img.Image image = Utils.getYUV420RotateImage(cameraImage,270);
  ...
  static registerFaceInDB(String name, List<double> embedding) {
